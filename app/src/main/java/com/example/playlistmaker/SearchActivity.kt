    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val simpleTextWatcher = object : TextWatcher {
            ...
            override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                if (binding.editText.hasFocus() && p0?.isEmpty() == true && items.isNotEmpty()) {
                    viewGroupTrackList2(VISIBLE)
                } else {
                ...
                if (!p0.isNullOrEmpty()) {
                    binding.buttonCleanSearch.visibility = VISIBLE
                } else {
                    binding.buttonCleanSearch.visibility = GONE
                }
            }
        ...
        binding.editText.setOnEditorActionListener { _, actionId, _ ->
            if (actionId == EditorInfo.IME_ACTION_DONE) {
                iTunesServiceSearch(binding.editText.text.toString())
                true
            }
            false
        }
